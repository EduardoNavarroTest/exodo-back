...
class UserDaoSQL {
    ...
    async findByCode(code) {
        try {
            ...
            console.log('Error finding user by code:', err);
            throw err;
    ...
    async updateById(id, newCode, newName, newDescription, newStatus) {
        try {
            ...
            const index = users.findIndex(user => user.id == id);
            if (index !== -1) {
                users[index] = { id: users[index].id, code: newCode, name: newName, description: newDescription, status: newStatus, user: users[index].user, date: users[index].date, userUpdate: users[index].userUpdate, dateUpdate: new Date() };
                await fs.writeFile(path, JSON.stringify(users, null, 2));
                return users[index];
            }
            return null;
